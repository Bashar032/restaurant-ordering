...
function getBookingArea(tableNumber: string): AreaId | null {
  const normalized = normalizeTableNumber(tableNumber);

  for (const area of areas) {
    const found = area.tables.some(
      (table) =>
        normalizeTableNumber(table.id) === normalized,
    );
...
export default function AdminFloorManager() {
  ...
  const [dayBookings, setDayBookings] =
    useState<AdminBooking[]>([]);

  const [showDayBookings, setShowDayBookings] =
    useState(false);

  const [dayBookingsLoading, setDayBookingsLoading] =
    useState(true);

  const [bookingAreaFilter, setBookingAreaFilter] =
    useState<"all" | AreaId>("all");
  ...
  const [notePhone, setNotePhone] =
    useState("");

  const [noteEmail, setNoteEmail] =
    useState("");
  ...
  const totalBookedTables = dayBookings.length;
  ...
  const filteredDayBookings =
    bookingAreaFilter === "all"
      ...
      : dayBookings.filter(
          (booking) =>
            getBookingArea(booking.table_number) ===
            bookingAreaFilter,
        );

  const filteredGuests = filteredDayBookings.reduce(
    (sum, booking) => sum + booking.guests,
    0,
  );

  const indoorBookings = dayBookings.filter(
    (booking) =>
      getBookingArea(booking.table_number) === "indoor",
  );

  const terraceBookings = dayBookings.filter(
    (booking) =>
      getBookingArea(booking.table_number) === "terrace",
  );

  const loungeBookings = dayBookings.filter(
    (booking) =>
      getBookingArea(booking.table_number) === "lounge",
  );

  const indoorGuests = indoorBookings.reduce(
    (sum, booking) => sum + booking.guests,
    0,
  );

  const terraceGuests = terraceBookings.reduce(
    (sum, booking) => sum + booking.guests,
    0,
  );

  const loungeGuests = loungeBookings.reduce(
    (sum, booking) => sum + booking.guests,
    0,
  );
  ...
  const loadBookings =
    useCallback(async () => {
      ...
      if (error) {
        setMessage(
          `Kunde inte hämta bokningar: ${error.message}`,
        );
  ...
  const loadDayBookings =
    useCallback(async () => {
      ...
      if (error) {
        setMessage(
          `Kunde inte hämta dagens bokningar: ${error.message}`,
        );
  ...
  useEffect(() => {
    ...
    setBookingAreaFilter("all");
  }, [loadDayBookings]);

  useEffect(() => {
    const channel = supabase
      .channel(
        `admin-floor-${bookingDate}-${bookingTime}`,
      )
      .on(
        "postgres_changes",
        {
  ...
  async function cancelBooking() {
    ...
    setMessage(
      "Bokningen har avbokats och bordet är nu ledigt.",
    );
  ...
  return (
    <section className="px-5 pb-20 pt-10 sm:px-8 lg:px-14">
      <div className="mx-auto max-w-[1600px]">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_auto]">
          <div className="border border-white/10 bg-white/[0.03] p-5">
            <p className="text-[0.62rem] font-bold uppercase tracking-[0.2em] text-white/40">
              Bokningar vald dag
            </p>

            <p className="mt-2 font-serif text-4xl text-[#D4B27C]">
              {dayBookingsLoading
                ? "..."
                : totalBookedTables}
            </p>
          ...
          <button
            ...
            className="min-h-20 border border-[#B08A52] bg-[#B08A52] px-7 text-xs font-bold uppercase tracking-[0.18em] text-[#191815] transition hover:bg-[#C29B61]"
          >
            {showDayBookings
              ? "Dölj bokade bord"
              : "Visa bokade bord"}
        ...
        {/* LISTA ÖVER BOKADE BORD */}

        {showDayBookings && (
          <div className="mt-4 overflow-hidden border border-white/10 bg-[#211E1A]">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 p-5 sm:p-6">
              <div>
                <p className="text-[0.62rem] font-bold uppercase tracking-[0.22em] text-[#B08A52]">
                  Bokade bord
                </p>
              ...
              <div className="text-right">
                <p className="font-serif text-3xl text-[#D4B27C]">
                  {filteredDayBookings.length}
                </p>

                <p className="text-xs text-white/40">
                  {filteredDayBookings.length === 1
                    ? "bokning"
                    : "bokningar"}{" "}
                  · {filteredGuests} gäster
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3 border-b border-white/10 p-5 sm:grid-cols-2 xl:grid-cols-4 sm:p-6">
              <button
                type="button"
                onClick={() =>
                  setBookingAreaFilter("all")
                ...
                className={`border px-5 py-4 text-left transition ${
                  bookingAreaFilter === "all"
                    ...
                    : "border-white/15 bg-white/[0.02] text-white hover:border-[#B08A52]"
                }`}
              >
                <span className="block text-[0.58rem] font-bold uppercase tracking-[0.18em] opacity-60">
                  Alla avdelningar
                </span>
                <span className="mt-2 block font-serif text-2xl">
                  {dayBookings.length} bord
                </span>
              ...
              <button
                type="button"
                onClick={() =>
                  setBookingAreaFilter("indoor")
                }
                className={`border px-5 py-4 text-left transition ${
                  bookingAreaFilter === "indoor"
                    ? "border-[#B08A52] bg-[#B08A52] text-[#191815]"
                    : "border-white/15 bg-white/[0.02] text-white hover:border-[#B08A52]"
                }`}
              >
                ...
                <span className="mt-1 block text-xs opacity-60">
                  {indoorBookings.length} bord · {indoorGuests} gäster
                </span>
              </button>
              ...
              <button
                type="button"
                onClick={() =>
                  setBookingAreaFilter("terrace")
                }
                className={`border px-5 py-4 text-left transition ${
                  bookingAreaFilter === "terrace"
                    ? "border-[#B08A52] bg-[#B08A52] text-[#191815]"
                    : "border-white/15 bg-white/[0.02] text-white hover:border-[#B08A52]"
                }`}
              >
                ...
                <span className="mt-1 block text-xs opacity-60">
                  {terraceBookings.length} bord · {terraceGuests} gäster
                </span>
              </button>
              ...
              <button
                type="button"
                onClick={() =>
                  setBookingAreaFilter("lounge")
                }
                className={`border px-5 py-4 text-left transition ${
                  bookingAreaFilter === "lounge"
                    ? "border-[#B08A52] bg-[#B08A52] text-[#191815]"
                    : "border-white/15 bg-white/[0.02] text-white hover:border-[#B08A52]"
                }`}
              >
                ...
                <span className="mt-1 block text-xs opacity-60">
                  {loungeBookings.length} bord · {loungeGuests} gäster
                </span>
              </button>
            </div>
            ...
            {dayBookingsLoading ? (
              ...
              <div className="p-10 text-center">
                ...
                <p className="mt-2 text-sm text-white/40">
                  Det finns inga bokningar för den valda avdelningen den här dagen.
                </p>
              </div>
            ) : (
              <div className="divide-y divide-white/10">
                {filteredDayBookings.map(
                  (booking) => (
                    <button
                      ...
                      className="grid w-full gap-4 p-5 text-left transition hover:bg-white/[0.04] sm:grid-cols-[120px_110px_1fr_130px] sm:items-center sm:px-6"
                    >
                      <div>
                        <p className="text-[0.58rem] font-bold uppercase tracking-[0.15em] text-white/35">
                          Bord
                        </p>

                        <p className="mt-1 font-serif text-2xl">
                          {booking.table_number.toUpperCase() === "LOUNGE"
                            ? "Lounge"
                            : booking.table_number}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="mt-1 font-serif text-2xl text-[#D4B27C]">
                          {booking.booking_time.slice(
                            0,
                            5,
                          )}
            ...
            {selectedTable ? (
                    ...
                    <div className="border border-[#D8CDBD] bg-white/60 p-5">
                      ...
                      <dl className="mt-5 space-y-3 text-sm">
                        ...
                        <div className="flex justify-between gap-5">
                          ...
                          <dd>
                            {selectedBooking.booking_time.slice(
                              0,
                              5,
                            )}
                          </dd>
              ...
              <div className="mt-6 border border-dashed border-[#CDBFAE] px-5 py-12 text-center">
                ...
                <p className="mx-auto mt-3 max-w-xs leading-7 text-[#676158]">
                  Klicka på ett rött
                  bord för kunduppgifter
                  eller ett grönt bord
                  för att skapa en
                  telefonbokning.
                </p>
